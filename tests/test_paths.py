...
from assos.paths import get_data_path, get_repository_path, get_visuals_path


def test_repository_runtime_paths(monkeypatch, tmp_path):
    monkeypatch.setenv("ASSOS_PATH", str(tmp_path))
...
def test_repository_path_is_required(monkeypatch):
    monkeypatch.delenv("ASSOS_PATH", raising=False)

    with pytest.raises(EnvironmentError, match="ASSOS_PATH"):
        get_repository_path()